...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalarProductsOfVectors = x1 * y1 + x2 * y2;
  const vectorModule1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorModule2 = Math.sqrt(x2 ** 2 + y2 ** 2);

  return (scalarProductsOfVectors / vectorModule1) * vectorModule2;
}
...
function roundToPowerOfTen(num, pow) {
  const powerOfRound = 10 ** pow;
  return Math.round(num, powerOfRound) * powerOfRound;
}
...
function getFibonacciNumber(index) {
  let numberFibonacci = 0;
  if (index === 0) numberFibonacci = index;
  else if (index === 1) numberFibonacci = index;
  else numberFibonacci = index - 2 + index - 1;
